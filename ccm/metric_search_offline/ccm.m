...
W_lower = sdpvar;
W_upper = sdpvar;
% ops = sdpsettings('solver','mosek','sos.numblkdg',1e-5); %, Need to try this
ops = sdpsettings('solver','mosek'); %, Need to try this
obj = W_upper; 

if isempty(state_set.W_states) % if W is a constant matrix, i.e. state-independent
    ...
    F = [M_pos>=0  W>= W_lower*eye(n) W_lower >= controller.W_lower_bound  W<=W_upper*eye(n)];
    sol = optimize(F,W_upper,ops);
    disp(sol.info);
    W_bar = value(W);
    max_residual = [];
    ...
    % Note that we have enforced that B_perp'*(dW_dbj - sym(dbj_dx*W(x)))B_perp' = 0 always holds, where bj
    ...
    paras = [paras;W_lower;W_upper;W_bar(:)];
    F = []; 
